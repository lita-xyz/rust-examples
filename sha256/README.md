...
This is a simple program that computes the SHA256 hash of a given input in Valida.
...
```
valida> valida run ./target/valida-unknown-baremetal-gnu/release/sha256 log
```

The `run` command runs the program and prints the SHA256 hash to the console and the file `log` in the current directory.
