...
This is a simple program that verifies a secp256k1 signature in Valida.
...
```
valida> valida run ./target/valida-unknown-baremetal-gnu/release/secp256k1 log
```

The `run` command runs the program and verifies the secp256k1 signature and prints the result to the console and the file `log` in the current directory.
